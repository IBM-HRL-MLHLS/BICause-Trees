...
def prevalence_symmetric(tree, alpha=0.1):
    ...
    ps_list = leaf_summary.iloc[:, 0].repeat(leaf_summary.iloc[:, 1])
...
def crump(tree, segments=10000):
    ...
    leaf_summary = tree.generate_leaf_summary() #series pscore,index
    ps_list = leaf_summary.iloc[:, 0].repeat(leaf_summary.iloc[:, 1])
    ps_vect = pd.DataFrame(np.column_stack((1 - ps_list, ps_list)))
...
class PropensityStartaficationPropensity():
    ...
    def fit_treatment_models(self,X,a):
        ...
        self.tree.get_positivity_violation_nodes(violating_nodes={})
        ...
        for node in total_nodes:
            ...
            has_positivity_violation = self.tree.violating_nodes[node]
...
class BecauseTree(IndividualOutcomeEstimator):
    ...
    def estimate_population_outcome(self, X, a, y=None, discard_violating_samples = True,treatment_values=None, agg_func="mean"):
    ...
    def estimate_individual_outcome(self,X,a,y=None,same_dim_as_input=True,treatment_values=None,predict_proba=None):
    ...
    def fit_outcome_models(self,X,a,y):
        ...
        for node in total_nodes:
            ...
            if has_positivity_violation:
                model = DummyEstimator().fit(X,a,y)
            else:
            # fit model to subset of samples
...
class PropensityImbalanceStratification(PopulationOutcomeEstimator):
    def __init__(
        self,
        asmd_threshold_split=0.1,
        min_leaf_size=0,
    ...
    def fit(self, X: pd.DataFrame, a: pd.Series):
        ...
        Returns: (object) root node of the fitted tree
        ...
        if self.positivity_filtering_method is not None:
            self.find_non_positivity_violating_leaves(positivity_filtering_method=self.positivity_filtering_method,
    ...
    def _calculate_asmds(X: pd.DataFrame, a: pd.Series):
        ...
        smds = (X0.mean() - X1.mean()) / ((np.sqrt(X0.var() + X1.var())) + epsilon)
        return smds.abs()
    ...
    def find_non_positivity_violating_leaves(self, positivity_filtering_method, positivity_filtering_kwargs: dict=None):
        """
    ...
    def get_positivity_violation_nodes(self,violating_nodes:dict={}):
        '''
        Creating a dictionay with all positivity violating nodes in the tree 
        '''

        if not self._is_leaf:
            self.subtree_[0].get_positivity_violation_nodes(violating_nodes)
            self.subtree_[1].get_positivity_violation_nodes(violating_nodes)
        else:
    ...
    def apply(self, X: pd.DataFrame):
        ...
        Returns: (pd.Index) leaf associated with each sample
