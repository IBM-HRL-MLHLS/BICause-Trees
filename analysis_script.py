...
def get_data(dataset):
    if dataset == 'twins':
        data = pd.read_csv('./twins_ztwins_sample0.csv')
        ...
        data = pd.read_csv('./natural_exp_dataset.csv')
        x = data.drop(columns=['T', 'D', 'y1', 'y0'])
        t = data['T']
        y = data['D']

    elif dataset == 'positivity_violations':
        data = pd.read_csv('./positivity_violations_data.csv')
        x = data.drop(columns=['T', 'D', 'y1', 'y0'])
        t = data['T']
        y = data['D']
